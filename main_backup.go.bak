...
var (
	...
	securityScan       bool // 安全扫描
	securityCheck      bool // 安全检查
	// 命令检测相关
	...
	for i := 0; i < len(p.args); i++ {
		...
		if strings.HasPrefix(arg, "-") && arg != "-" {
			...
			if len(arg) > 2 && arg[0] == '-' && arg[1] != '-' {
				// 这是短标志组合，如 -rf
				for j := 1; j < len(arg); j++ {
					singleFlag := "-" + string(arg[j])
					if p.isKnownFlag(singleFlag) {
						result.Flags = append(result.Flags, singleFlag)
					} else {
						return nil, fmt.Errorf(T("未知标志: %s (在 %s 中)"), singleFlag, arg)
					}
				}
			} else if p.isKnownFlag(arg) {
...
func (p *SmartArgumentParser) isKnownFlag(arg string) bool {
	...
	delguardFlags := map[string]bool{
		...
		"--config": true, "--restore": true, "--security-scan": true, "--security-check": true,
		// 短参数支持
...
func (p *SmartArgumentParser) createUnknownFlagError(arg string) error {
	msg := fmt.Sprintf(T("未知标志: %s"), arg)

	// 根据模式提供不同的建议
	switch p.mode {
	case ModeDel:
		msg += "\n" + T("提示：如果这是文件名，请使用 del -- ") + arg
	case ModeRM:
		msg += "\n" + T("提示：如果这是文件名，请使用 rm -- ") + arg
	case ModeCP:
		msg += "\n" + T("提示：cp命令不支持此标志")
	default:
		msg += "\n" + T("提示：如果这是文件名，请使用 delguard -- ") + arg
	}
...
func main() {
	...
	flag.BoolVar(&securityScan, "security-scan", false, "执行安全扫描")
	flag.BoolVar(&securityCheck, "security-check", false, "执行安全检查")
	// 智能删除参数
	...
	// 处理解析出的标志参数
	for _, flagArg := range preResult.Flags {
		switch flagArg {
		case "-v", "--verbose":
			verbose = true
		case "-q", "--quiet":
		...
		case "--force-confirm":
			forceConfirm = true
		case "--install":
			installAliasOnly = true
		case "--uninstall":
			uninstallAliasOnly = true
		case "--version":
			showVersion = true
		case "-h", "--help":
			showHelp = true
	...
	if securityScan {
		paths := flag.Args()
		if len(paths) == 0 {
			fmt.Println(T("请提供要检查的文件或目录路径"))
			return
		}
		if len(paths) > 0 {
			result := RunSimpleSecurityCheck(paths[0])
			fmt.Printf(T("安全检查完成: %d个文件，%d个警告\n"), result.FilesScanned, result.Warnings)
		} else {
			fmt.Println(T("请提供要检查的文件或目录路径"))
		}
		return
	...
	if protect {
		if err := EnableOverwriteProtection(); err != nil {
			if !HandleError(err, "启用覆盖保护", false) {
				return
			}
		}
	...
	if disableProtect {
		if err := DisableOverwriteProtection(); err != nil {
			if !HandleError(err, "禁用覆盖保护", false) {
				return
			}
		}
	...
	// 显示帮助
	if showHelp {
		helpSystem := NewHelpSystem(currentMode, nil)
		helpSystem.ShowHelp()
		return
	}
	...
	if flag.NArg() > 0 && flag.Arg(0) == "restore" {
		...
		if err := restoreFlagSet.Parse(flag.Args()[1:]); err != nil {
			if !HandleError(err, "恢复参数解析", false) {
				return
			}
		}
	...
	for _, file := range files {
		...
		if _, err := sanitizeFileName(file); err != nil {
			...
			logger.Error("文件通配符展开失败", file, dgErr, "输入路径包含非法字符或格式")
			preErrCount++
			continue
		}
		...
		if err != nil {
			...
			logger.Error("文件通配符展开失败", file, dgErr, "通配符展开失败")
			preErrCount++
			continue
		}
		...
		for _, expFile := range expanded {
			...
			if err != nil {
				dgErr := WrapE("resolveAbsPath", expFile, err)
				logger.Error("无法解析文件路径", expFile, dgErr, "解析绝对路径失败")
				preErrCount++
				continue
			}
			...
			if err != nil {
				if os.IsNotExist(err) {
					...
					logger.Error("无法访问文件", abs, err, "无法访问文件")
					preErrCount++
					continue
				}
			}
		...
		validateSmartFile:
			...
			if err := checkFileType(abs); err != nil {
				...
				logger.Error("不支持删除特殊文件类型", abs, dgErr, "不支持删除特殊文件类型")
				preErrCount++
				continue
			}
	...
	if preErrCount > 0 {
		HandleError(fmt.Errorf("预处理阶段发现 %d 个错误", preErrCount), "文件预处理", true)
		return
	}
	...
	if validateOnly {
		...
		if validCount != len(results) {
			...
			HandleError(fmt.Errorf("验证失败: %d/%d 文件未通过验证", len(results)-validCount, len(results)), "文件验证", true)
			return
		} else {
...
// printUsage 显示使用帮助
func printUsage() {
	help := `DelGuard - 安全删除工具

用法:
  delguard [选项] <文件路径...>
  delguard restore [选项] [模式>
  delguard --cp [选项] <源文件> <目标文件>

主要选项:
  -f, --force        强制删除，跳过确认
  -i, --interactive  交互模式，逐个确认
  -r, --recursive    递归删除目录
  -v, --verbose      详细输出
      --dry-run      仅验证，不实际删除
      --protect      启用文件覆盖保护
...
智能删除选项与示例:
  delguard file.txt               # 删除文件到回收站
  delguard -f *.tmp               # 强制删除所有.tmp文件
  delguard -i folder/             # 交互式删除目录
  delguard test_file              # 智能搜索相似文件名
  delguard *.txt --force-confirm  # 批量删除跳过确认
  delguard --search-content doc   # 搜索文件内容
  delguard restore file.txt       # 恢复指定文件
  delguard restore -l             # 列出所有可恢复文件
  cp file.txt backup.txt          # 安全复制文件
  cp -r folder/ backup/           # 递归复制目录

注意:
  DelGuard 会将文件移动到系统回收站，不会直接删除。
  cp 命令会安全处理文件覆盖，将原文件移入回收站。`
	fmt.Println(T(help))
}
...
func handleCopyCommand() {
	...
	// 干跑模式（与 del/rm 行为保持一致）
	cpFlag.BoolVar(&dryRun, "n", false, "干跑模式（不实际复制，只显示将执行的操作）")
	cpFlag.BoolVar(&dryRun, "dry-run", false, "干跑模式（不实际复制，只显示将执行的操作）")
	cpFlag.BoolVar(&dryRun, "no-clobber", false, "干跑模式（不实际复制，只显示将执行的操作）")
...
选项:
  ...
  -v, --verbose        详细输出
  -p, --preserve       保留文件属性
  -n, --no-clobber     干跑模式（不实际复制）
      --dry-run        干跑模式（不实际复制）`
		fmt.Println(T(cpHelp))
		os.Exit(1)
	}
	...
	if len(sources) > 1 && !isDestDir {
		HandleError(fmt.Errorf("目标 '%s' 不是目录", dest), "参数验证", false)
		return
	}
	...
	if failCount > 0 {
		HandleError(fmt.Errorf("没有提供文件参数"), "参数检查", true)
		return
	}
	...
	if len(files) == 0 {
		...
		HandleError(fmt.Errorf("没有找到匹配的文件"), "文件查找", true)
		return
	}

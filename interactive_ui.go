...
func (ui *InteractiveUI) ShowError(err error, suggestion string) {
	logger.Error("交互界面错误", "error", err, "suggestion")
	if suggestion != "" {
		fmt.Printf(T(" 建议: %s\n"), suggestion)
	}
}
...
func (ui *InteractiveUI) ShowSmartError(err error, context string) {
	...
	errorMsg := err.Error()
	switch {
	case strings.Contains(errorMsg, "no such file") || strings.Contains(errorMsg, "") || strings.Contains(errorMsg, "not exist"):
		fmt.Printf(T(" 建议：\n"))
	...
	case strings.Contains(errorMsg, "permission") || strings.Contains(errorMsg, ""):
		fmt.Printf(T(" 建议：\n"))
		fmt.Printf(T("   1. 以管理员身份运行\n"))
		fmt.Printf(T("   2. 检查文件权限设置\n"))
		fmt.Printf(T("   3. 确认文件没有被其他程序占用\n"))
	case strings.Contains(errorMsg, "invalid") || strings.Contains(errorMsg, ""):
		fmt.Printf(T(" 建议：\n"))
		fmt.Printf(T("   1. 检查文件名中是否包含非法字符\n"))
		fmt.Printf(T("   2. 避免使用特殊字符如 < > | \" : * ? \\ /\n"))
		fmt.Printf(T("   3. 使用引号包围包含空格的文件名\n"))
	case strings.Contains(errorMsg, "too long") || strings.Contains(errorMsg, ""):
		fmt.Printf(T(" 建议：\n"))
		fmt.Printf(T("   1. 缩短文件路径或文件名\n"))
		fmt.Printf(T("   2. 移动到更短的目录路径\n"))
	default:
		logger.Info("操作建议", "message", T("请检查错误信息并尝试重新操作"))
	}
}
